...
# Build 61 — RPPairing multihost fallback.
# LocalDevVPN forwards the RSD entry point at 10.7.0.1:49152, but the
# RPPairing handshake may open a second, dynamically allocated tunnel port.
# Some LocalDevVPN versions expose that second listener on the iPhone's
# Wi-Fi address rather than through the VPN peer. Retry the *same* RPPairing
# operation against the phone's own non-loopback IPv4 addresses as well.
def patch_build61_multihost():
    p = ROOT / "Locus/Engine/LocationEngine.swift"
    if not p.exists():
        raise SystemExit("Build61: LocationEngine.swift not found")
    c = p.read_text(encoding="utf-8")
    import re

    # Replace the final Build60 setLocked implementation.
    new_func = r'''    private static func setLocked(latitude: Double, longitude: Double, pairingPath: String, deviceIP: String) -> Int32 {
        if let locationSimulation {
            if let err = location_simulation_set(locationSimulation, latitude, longitude) {
                idevice_error_free(err)
        ...
        // Build a deterministic candidate list. 10.7.0.1 is the normal
        // LocalDevVPN peer. If the VPN only forwards the fixed RSD listener
        // while the dynamic RPPairing listener is reachable on Wi-Fi, retry
        // through the iPhone's own Wi-Fi address. Loopback is last.
        var hosts: [String] = [deviceIP]
        ...
        if getifaddrs(&ifaddr) == 0, let first = ifaddr {
            ...
            while let current = ptr {
                ...
                if let sa = a.ifa_addr, sa.pointee.sa_family == UInt8(AF_INET) {
                    ...
                    if !ip.isEmpty && ip != "127.0.0.1" && !ip.hasPrefix("10.7.0.") && !hosts.contains(ip) {
                        hosts.append(ip)
                    }
                }
                ptr = a.ifa_next
        ...
        for host in hosts {
            ...
            if let providerError {
                let message = providerError.pointee.message.map { String(cString: $0) } ?? "unknown"
                lastMessage = "\(host): code=\(providerError.pointee.code) sub=\(providerError.pointee.sub_code) \(message)"
                NSLog("DPort RPPairing candidate failed: %@", lastMessage)
                idevice_error_free(providerError)
                cleanup()
                continue
            }
            ...
            if adapter != nil && handshake != nil {
                NSLog("DPort RPPairing connected via candidate %@", host)
                tunnelOK = true
                break
            }
            cleanup()
        ...
        if !tunnelOK {
            NSLog("DPort RPPairing all candidates failed: %@", lastMessage)
            return tunnelCreate
        }

        if let remoteServerError = remote_server_connect_rsd(adapter, handshake, &remoteServer) {
            let message = remoteServerError.pointee.message.map { String(cString: $0) } ?? "unknown"
            NSLog("DPort RSD handshake failed: code=%d sub=%d message=%@", remoteServerError.pointee.code, remoteServerError.pointee.sub_code, message)
            idevice_error_free(remoteServerError)
            cleanup()
            return remoteServerCode
        }
        ...
        if let simError = location_simulation_new(remoteServer, &locationSimulation) {
            let message = simError.pointee.message.map { String(cString: $0) } ?? "unknown"
            NSLog("DPort location simulation creation failed: %@", message)
            idevice_error_free(simError)
            cleanup()
            return simulationCreate
        }
...
'''
    ...
    m = pat.search(c)
    if not m:
        raise SystemExit("Build61: setLocked function not found")
    c = c[:m.start()] + new_func + '    private static func clearLocked' + c[m.end():]
    ...
    if project.exists():
        ps = project.read_text(encoding="utf-8")
        ps = re.sub(r'CURRENT_PROJECT_VERSION:\\s*"\\d+"', 'CURRENT_PROJECT_VERSION: "61"', ps)
        project.write_text(ps, encoding="utf-8")
...
# Build 62 API compatibility fix for SideInstaller FFI.
def patch_build62_api_compat():
    import re

    # The pinned SideInstaller FFI exposes the 7-argument pairable_host_accept.
    # Replace the entire call body instead of relying on whitespace-sensitive regex.
    q = ROOT / "Locus/Engine/PairOnDeviceService.swift"
    if q.exists():
        c = q.read_text(encoding="utf-8")
        marker = "pairable_host_accept("
        pos = c.find(marker)
        if pos < 0:
            raise SystemExit("Build62: pairable_host_accept call not found")
        open_paren = pos + len(marker) - 1
        depth = 0
        close = -1
        for k in range(open_paren, len(c)):
            if c[k] == "(":
                depth += 1
            elif c[k] == ")":
                depth -= 1
                if depth == 0:
                    ...
                    break
        if close < 0:
            raise SystemExit("Build62: pairable_host_accept closing parenthesis not found")
        new_call = """pairable_host_accept(
                    namePtr,
                    modelPtr,
                    0,
        ...
        c = c[:pos] + new_call + c[close + 1:]
        q.write_text(c, encoding="utf-8")

    # multihost takes const char *const *.
    p = ROOT / "Locus/Engine/LocationEngine.swift"
    if p.exists():
        c = p.read_text(encoding="utf-8")
        c = c.replace(
...
def patch_build63_coredevice():
    ...
    new_func = r'''    private static func setLocked(latitude: Double, longitude: Double, pairingPath: String, deviceIP: String) -> Int32 {
        ...
        let cacheDir = appSupport.appendingPathComponent("Pairing", isDirectory: true)
        let lockdownPath = cacheDir.appendingPathComponent("lockdown_pair_record.plist").path
        try? FileManager.default.createDirectory(at: cacheDir, withIntermediateDirectories: true)

        do {
            if FileManager.default.fileExists(atPath: lockdownPath),
               try connectCoreDeviceProxy(latitude: latitude, longitude: longitude,
                                          lockdownPath: lockdownPath, deviceIP: deviceIP) {
                return ok
            }
        } catch {
            NSLog("[DPort] cached CoreDevice pair record failed: %@", String(describing: error))
            cleanup()
        }

        do {
            let record = try mintLockdownPairRecord(hosts: [deviceIP, "127.0.0.1"])
            try record.write(to: URL(fileURLWithPath: lockdownPath), options: .atomic)
            try FileManager.default.setAttributes([.posixPermissions: 0o600], ofItemAtPath: lockdownPath)
            NSLog("[DPort] lockdown pair record minted: %lu bytes", record.count)

            if try connectCoreDeviceProxy(latitude: latitude, longitude: longitude,
                                          lockdownPath: lockdownPath, deviceIP: deviceIP) {
                return ok
            }
        } catch {
            NSLog("[DPort] CoreDeviceProxy route failed: %@", String(describing: error))
        }
    ...
    private static func connectCoreDeviceProxy(latitude: Double, longitude: Double,
                                               lockdownPath: String, deviceIP: String) throws -> Bool {
        var pairing: OpaquePointer?
        if let err = lockdownPath.withCString({ idevice_pairing_file_read($0, &pairing) }) {
            let message = err.pointee.message.map { String(cString: $0) } ?? "unknown"
            idevice_error_free(err)
            NSLog("[DPort] lockdown pairing read failed: %@", message)
            return false
        }
        ...
        let providerError = withUnsafePointer(to: &address) { pointer in
            pointer.withMemoryRebound(to: sockaddr.self, capacity: 1) { sa in
                "DPort".withCString { label in
                    idevice_tcp_provider_new(sa, pairing, label, &provider)
                }
            }
        }
        if let providerError {
            ...
            let message = providerError.pointee.message.map { String(cString: $0) } ?? "unknown"
            idevice_error_free(providerError)
            NSLog("[DPort] TCP provider failed: %@", message)
            return false
        }
        ...
        var newAdapter: OpaquePointer?
        var newHandshake: OpaquePointer?
        if let tunnelError = tunnel_create_usb(provider, &newAdapter, &newHandshake) {
            let message = tunnelError.pointee.message.map { String(cString: $0) } ?? "unknown"
            idevice_error_free(tunnelError)
            NSLog("[DPort] CoreDeviceProxy tunnel failed: %@", message)
            return false
        }
        guard let newAdapter, let newHandshake else { return false }

        cleanup()
        adapter = newAdapter
        handshake = newHandshake

        if let remoteError = remote_server_connect_rsd(adapter, handshake, &remoteServer) {
            let message = remoteError.pointee.message.map { String(cString: $0) } ?? "unknown"
            idevice_error_free(remoteError)
            NSLog("[DPort] RSD over CoreDeviceProxy failed: %@", message)
            cleanup()
            return false
        }
        ...
        if let simError = location_simulation_new(remoteServer, &locationSimulation) {
            let message = simError.pointee.message.map { String(cString: $0) } ?? "unknown"
            idevice_error_free(simError)
            NSLog("[DPort] location simulation service failed: %@", message)
            cleanup()
            return false
        }
        remoteServer = nil
        ...
        if let setError = location_simulation_set(locationSimulation, latitude, longitude) {
            let message = setError.pointee.message.map { String(cString: $0) } ?? "unknown"
            idevice_error_free(setError)
            NSLog("[DPort] location set failed: %@", message)
            cleanup()
            return false
        }
        NSLog("[DPort] LOCATION_SET_OK via CoreDeviceProxy")
        return true
    }

    private static func mintLockdownPairRecord(hosts: [String]) throws -> Data {
        let hostIDKey = "DPort.lockdownHostID"
        let systemBUIDKey = "DPort.lockdownSystemBUID"
        let hostID = (UserDefaults.standard.string(forKey: hostIDKey) ?? UUID().uuidString.uppercased())
        let systemBUID = (UserDefaults.standard.string(forKey: systemBUIDKey) ?? UUID().uuidString.uppercased())
        UserDefaults.standard.set(hostID, forKey: hostIDKey)
        UserDefaults.standard.set(systemBUID, forKey: systemBUIDKey)

        var lastMessage = "unknown"
        for host in hosts {
            var address = sockaddr_in()
            address.sin_family = sa_family_t(AF_INET)
            address.sin_port = in_port_t(62078).bigEndian
            guard host.withCString({ inet_pton(AF_INET, $0, &address.sin_addr) }) == 1 else { continue }

            var device: OpaquePointer?
            let connectError = withUnsafePointer(to: &address) { pointer in
                pointer.withMemoryRebound(to: sockaddr.self, capacity: 1) { sa in
                    "DPortPair".withCString { label in
                        idevice_new_tcp_socket(sa, socklen_t(MemoryLayout<sockaddr_in>.stride), label, &device)
                    }
                }
            }
            if let connectError {
                lastMessage = connectError.pointee.message.map { String(cString: $0) } ?? "unknown"
            ...
            if let newError = lockdownd_new(device, &client) {
                lastMessage = newError.pointee.message.map { String(cString: $0) } ?? "unknown"
                idevice_error_free(newError)
                continue
            }
            guard let client else { continue }
            defer { lockdownd_client_free(client) }

            var pairFile: OpaquePointer?
            let pairError = hostID.withCString { h in
                systemBUID.withCString { b in
                    "DPort".withCString { name in
                        lockdownd_pair(client, h, b, name, &pairFile)
                    }
                }
            }
            if let pairError {
                lastMessage = pairError.pointee.message.map { String(cString: $0) } ?? "unknown"
                idevice_error_free(pairError)
                continue
            }
            guard let pairFile else { continue }
            defer { idevice_pairing_file_free(pairFile) }

            var bytes: UnsafeMutablePointer<UInt8>?
            var length: UInt = 0
            if let serializeError = idevice_pairing_file_serialize(pairFile, &bytes, &length) {
                lastMessage = serializeError.pointee.message.map { String(cString: $0) } ?? "unknown"
                idevice_error_free(serializeError)
                continue
            }
        ...
        throw NSError(domain: "DPort", code: Int(tunnelCreate),
